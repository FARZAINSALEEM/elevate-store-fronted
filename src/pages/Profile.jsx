...
const Profile = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [ordersRes, wishlistRes] = await Promise.all([
        api.get('/orders/'),
        api.get('/wishlists/') // Note: Ensure you map this to the correct endpoint in urls.py if different
      ]);
      setOrders(ordersRes.data);
      // Assuming the API returns a list of wishlists, and we want the current user's (usually just one)
  ...
  const removeFromWishlist = async (productId) => {
    ...
    try {
       // Note: Adjust this payload based on exactly how your backend expects it.
       // Easiest is usually a specific endpoint to remove an item, or patching the whole list
  ...
  const downloadPDF = async (orderId) => {
    try {
      // Assuming you create an endpoint to fetch the raw PDF bytes or base64
                      ...
                      return (
                        <div key={order.id} className="bg-neutral-900/40 border border-neutral-800 rounded-3xl p-6 md:p-8 overflow-hidden relative shadow-lg">
                          ...
                          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-2 pb-6 border-b border-neutral-800/80">
                            ...
                            <div className="mt-4 md:mt-0 flex gap-3">
                                {order.status !== 'CANCELLED' && order.status !== 'PENDING' && (
                                   <button onClick={() => downloadPDF(order.id)} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-neutral-700 hover:border-indigo-500 text-sm font-bold text-neutral-300 hover:text-white bg-neutral-800 hover:bg-neutral-700 transition-all">
                          ...
                          {order.status === 'CANCELLED' && order.rejection_reason && (
                            <div className="mb-6 p-5 bg-red-500/10 border border-red-500/30 rounded-2xl flex gap-4 text-red-400 text-sm">
                                ...
                                {order.items?.map(item => (
                                  <div key={item.id} className="flex justify-between items-center text-sm">
                                    <span className="text-neutral-300 flex items-center gap-3">
                                      <span className="bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 px-2 py-1 rounded text-xs font-black">{item.quantity}x</span> 
                          ...
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-neutral-900 p-5 rounded-2xl border border-neutral-800">
                            ...
                            <div className="text-left sm:text-right w-full sm:w-auto">
                              ...
                              <span className="text-3xl font-black text-indigo-400 tracking-tight">Rs. {order.total_price}</span>
                            </div>
                          </div>
                        </div>
                      );
                ...
                {loading ? (
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {wishlist.map(product => (
                         <div key={product.id} className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-4 relative group">
                            <button onClick={() => removeFromWishlist(product.id)} className="absolute top-6 right-6 z-10 p-2 bg-red-500/20 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100">
                               <Trash2 size={16}/>
                            </button>
                            <Link to={`/product/${product.id}`} className="block h-40 bg-black rounded-xl mb-4 overflow-hidden">
                               {product.image ? <img src={getImageUrl(product.image)} alt={product.name} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-neutral-700">No Image</div>}
                            </Link>
